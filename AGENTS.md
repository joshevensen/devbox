...
This is **not a code project** — it's the user's remote devbox. All real coding happens inside repos under `~/devbox/repos/<project>/`. Work happens as the `dev` user; root is for system administration only.
...
| `~/devbox/repos/` | All project repositories. `cd` into one for project work. |
...
| `~/devbox/repos/<name>/` | Per-repo devbox config (`<name>.yaml`) alongside code repos. |
...
- **Per-project context** (build commands, schema notes, gotchas) belongs in that project's own `CLAUDE.md` inside `~/devbox/repos/<project>/`, not here.
...
- Server admin / setup tasks: install services, edit configs, manage systemd units (use `sudo`).
- Doc maintenance: update files in `~/devbox/docs/`.
- Cross-project utilities: shell aliases, helper scripts.

For anything inside a specific repo, expect to `cd ~/devbox/repos/<project>` first.
